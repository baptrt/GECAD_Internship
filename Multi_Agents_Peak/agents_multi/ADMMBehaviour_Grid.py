...
class ADMMBehaviour_Test(OneShotBehaviour):
    def __init__(self, a, b, n_agents, tmin, tmax, pmin, pmax, neighbors, rho, rhol, max_iters=500, max_error=1e-3):
        ...
        self.gamma_policy = {}
        self.last_gamma = np.zeros_like(self.T)    
        self.errors = []
            ...
            if msg:
                ...
                if msg_type == "state":
                    try:
                        ...
                        j = int(data["id"])
                        print(f"[{self.agent.name}] Received message from {j} of type {msg_type}")
    ...
    def _get_obs(self, T, prices):
        ...
        return np.concatenate([
            (self.last_gamma[1:, 1:]/self.max_gamma).flatten(), # previous gamma (flattened matrix)
            T[:, -1],              # last power exchanged (off-grid)
            prices[:, -1],         # last prices traded
            np.sum(T, axis=1),     # total power exchanged
            np.sum(prices, axis=1) # total price exchanged      
        ]).astype(np.float32)
    ...
    async def broadcast_gamma(self, i):
        await asyncio.sleep(2.0)

        gamma = self.update_gamma_policy(i)
        gamma = np.insert(gamma, 0, 0.0, axis=0)
        gamma = np.insert(gamma, 0, 0.0, axis=1)

        # Full local update
        self.gamma = gamma.copy()

        for j in self.neighbors:
            if j == 0:  # Skip self and grid
                continue
            ...
            for n in range(self.n_agents):
                if n == 0 or j == n:
                    continue
                key = f"{j},{n}"
                gamma_dict[key] = float(gamma[j, n])

            # Send a single message with the full dictionary
            msg = Message(to=f"agent_{j}@xmpp.gecad.isep.ipp.pt")
            msg.set_metadata("performative", "inform")
            msg.set_metadata("type", "gamma_update")
            msg.body = json.dumps({
                "from": i,
                "to": j,
            ...
            await self.send(msg)
            print(f"[Agent {i}] Sent gamma_dict to agent {j}: {gamma_dict}")
    ...
    async def run(self):
        ...
        while global_iter < 10 * self.max_iters and error > self.max_error:
            ...
            await self.send_state_to_neighbors(i)
            await self.broadcast_gamma(i)
            await asyncio.sleep(1.0)
            
            self.last_gamma = self.gamma.copy()
